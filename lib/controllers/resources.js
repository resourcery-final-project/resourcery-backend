...
module.exports = Router()
  ...
  .get('/order/asc', async (req, res, next) => {
    try {
      const resources = await Resource.getOrderedAsc();
      res.send(resources);
    } catch (error) {
      next(error);
    }
  ...
  .get('/order/desc', async (req, res, next) => {
    try {
      const resources = await Resource.getOrderedDsc();
      res.send(resources);
    } catch (error) {
      next(error);
    }
  ...
  .get('/count', async (req, res, next) => {
    try {
      const resources = await Resource.getTotalResources();
